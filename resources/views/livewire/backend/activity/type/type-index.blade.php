    ...
    <div class="mb-4 flex flex-col gap-2 px-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div class="mt-3 sm:mt-0">              
            <flux:button wire:navigate href="{{ route('activity.type.create') }}" size="sm">{{ __('Add Type') }}</flux:button>         
        </div>
      </div>
    </div>
    ...
    <div class="custom-scrollbar max-w-full overflow-x-auto px-5 sm:px-6">
      <table class="min-w-full table-auto">
        <thead class="border-y border-gray-100 py-3 dark:border-gray-800">
          <tr>
            <th class="py-3 font-normal text-left whitespace-nowrap">Type Name</th>
          ...
          @forelse ($activities as $activity)
            <tr class="align-top">
              <td class="py-4 px-3 whitespace-nowrap text-gray-800 dark:text-white">
                {{$activity->activity_type}} ( {{$activity->activities->count()}} )
              </td>
              ...
              <td class="py-4 px-3 whitespace-nowrap  text-center">
                <div class="flex gap-2">
                    ...
                    <flux:button size="xs" variant="primary" icon="pencil" wire:navigate href="{{ route('activity.type.edit', $activity->id) }}" aria-label="Edit Activity" />
                    {{-- @if(!$admin->hasRole('super-admin') && auth()->user()->can('activity.delete'))       --}}
